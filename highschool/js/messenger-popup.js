...
<div id="msg-popup-overlay" class="hidden fixed inset-0 z-[100] bg-black/40 backdrop-blur-sm flex items-center justify-center p-4" onclick="if(event.target===this)closeMsgPopup()">
  <div class="bg-white rounded-2xl shadow-2xl w-full max-w-2xl flex flex-col overflow-hidden" style="height:min(80vh,700px)">
    ...
    function _loadRooms(){
        fetch('/api/message/rooms').then(r=>r.json()).then(data=>{
            ...
            rooms.forEach(r=>{
                ...
                h+=`<div onclick="openMsgChat(${r.id})" class="flex items-center gap-3 px-4 py-3 hover:bg-slate-50 cursor-pointer transition ${u>0?'bg-blue-50/50':''}">
